"""
CredTech Intelligence Platform - Simple Dashboard
=================================================

A streamlined credit dashboard for deployment with minimal dependencies.
"""
...
def main():
    ...
    if uploaded_file is not None:
        ...
        sample_size = st.sidebar.slider("Sample Data Size", 100, 2000, 500, 100, key="sample_size_slider")
        if st.sidebar.button("🔄 Generate New Sample", key="generate_sample_btn"):
        ...
        try:
            ...
            if isinstance(st.session_state.shap_values, list):
                shap.plots.waterfall(
                    shap.Explanation(
                        values=st.session_state.shap_values[0][client_idx],
                        base_values=st.session_state.shap_explainer.expected_value[0],
                        data=st.session_state.X_test.iloc[client_idx]
                    ),
                    show=False
                )
            else:
                shap.plots.waterfall(
                    shap.Explanation(
                        values=st.session_state.shap_values[client_idx],
            ...
            st.info("Waterfall plot requires newer version of SHAP. Showing alternative explanation.")
            
            # Alternative: Feature values and their SHAP values
            if isinstance(st.session_state.shap_values, list):
